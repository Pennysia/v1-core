...
contract MarketTest is Test {
    ...
    function test_ConstructorSetsOwner() public view {
        assertEq(
            market.owner(),
            owner,
            "Constructor should set owner correctly"
        );
    }
    ...
    function test_SetOwnerOnlyByOwner() public {
        ...
        assertEq(
            market.owner(),
            newOwner,
            "setOwner should update owner when called by current owner"
        );
    ...
    function test_GetPairIdComputesCorrectId() public view {
        ...
        assertEq(
            pairId,
            expectedId,
            "getPairId should compute correct keccak hash for sorted tokens"
        );
    }
    ...
    function test_GetPairIdEdgeZeroAddresses() public view {
        ...
        assertTrue(
            pairId != 0,
            "getPairId should handle zero address and compute non-zero ID"
        );
    }
    ...
    function test_GetReservesReturnsZerosForNonExistentPair() public view {
        ...
        (uint128 r0L, uint128 r0S, uint128 r1L, uint128 r1S) = market
    ...
    function test_GetSweepableComputesCorrectAmount() public {
        ...
        assertEq(
            sweepable,
            400,
            "getSweepable should return balance - tracked balance"
        );
    }
    ...
    function test_GetSweepableWithZeroBalances() public {
        ...
        assertEq(
            sweepable,
            0,
            "getSweepable should return 0 when balances are zero"
        );
    }
    ...
    function test_SweepByOwnerMultipleTokens() public {
        ...
        vm.store(
            address(market),
            keccak256(abi.encode(token1, mappingSlot)),
            bytes32(uint256(300))
        );
        vm.store(
            address(market),
            keccak256(abi.encode(token2, mappingSlot)),
            bytes32(uint256(100))
        );
        ...
        assertEq(
            mockToken1.balanceOf(address(market)),
            300,
            "Balance after sweep should reflect transfer for token1"
        );
        assertEq(
            mockToken2.balanceOf(address(market)),
            100,
            "Balance after sweep should reflect transfer for token2"
        );
    }
    ...
    function test_SweepRevertsOnExcessiveAmounts() public {
        ...
        vm.store(
            address(market),
            keccak256(abi.encode(token, mappingSlot)),
            bytes32(uint256(50))
        ); // tracked tokenBalances[token] = 50
    ...
    function test_SweepEdgeMaxUint() public {
        ...
        vm.store(
            address(market),
            keccak256(abi.encode(token, uint256(keccak256("tokenBalances")))),
            bytes32(uint256(1))
        );
    ...
    function test_FlashSingleToken() public {
        ...
        require(
            mockToken.balanceOf(address(callback)) >= expectedPayback,
            "Callback does not have enough balance"
        );

        uint256 initialMarketBalance = mockToken.balanceOf(address(market));
        emit log_named_uint(
            "Market balance before flash",
            initialMarketBalance
        );
        ...
        assertEq(
            finalMarketBalance,
            initialMarketBalance + expectedFee,
            "Market balance should increase by fee"
        );
    }
    ...
    function test_FlashMultiToken() public {
        ...
        require(
            mockToken1.balanceOf(address(callback)) >= payback1,
            "Callback does not have enough balance for token1"
        );
        require(
            mockToken2.balanceOf(address(callback)) >= payback2,
            "Callback does not have enough balance for token2"
        );
        ...
        emit log_named_uint(
            "Token1 market balance before flash",
            initialMarketBalance1
        );
        emit log_named_uint(
            "Token2 market balance before flash",
            initialMarketBalance2
        );
        ...
        emit log_named_uint(
            "Token1 market balance after flash",
            finalMarketBalance1
        );
        emit log_named_uint(
            "Token2 market balance after flash",
            finalMarketBalance2
        );

        assertEq(
            finalMarketBalance1,
            initialMarketBalance1 + fee1,
            "Token1 balance should increase by fee"
        );
        assertEq(
            finalMarketBalance2,
            initialMarketBalance2 + fee2,
            "Token2 balance should increase by fee"
        );
    }
    ...
    function test_FlashFuzz(uint256 amount) public {
        ...
        require(
            mockToken.balanceOf(address(callback)) >= payback,
            "Callback does not have enough balance"
        );

        uint256 initialMarketBalance = mockToken.balanceOf(address(market));
        emit log_named_uint(
            "Market balance before flash (fuzz)",
            initialMarketBalance
        );
        ...
        uint256 finalMarketBalance = mockToken.balanceOf(address(market));
        emit log_named_uint(
            "Market balance after flash (fuzz)",
            finalMarketBalance
        );
        ...
        assertEq(
            finalMarketBalance,
            initialMarketBalance + expectedFee,
            "Balance should increase by fee"
        );
    }
    ...
    function test_CreateLiquidityExistingPair() public {
        ...
        vm.prank(address(callback));
        (, uint256 liq0L, , , ) = market.createLiquidity(
            to,
            token0,
            token1,
            add0L,
    ...
    function test_CreateLiquidityFuzz(
        uint256 a0L,
        uint256 a0S,
        uint256 a1L,
        uint256 a1S
        ...
        (uint128 r0L, uint128 r0S, uint128 r1L, uint128 r1S) = market
            .getReserves(token0, token1);
        assertTrue(
            r0L > 0 && r0S > 0 && r1L > 0 && r1S > 0,
            "Reserves should be >0 after add"
        );
    }
    ...
    function test_WithdrawLiquiditySuccess() public {
        ...
        TestLiquidityCallback liquidityCallback = new TestLiquidityCallback(
            market
        );
        address to = address(0xDEF);
        ...
        address token0 = address(mockTokenA) < address(mockTokenB)
            ? address(mockTokenA)
            : address(mockTokenB);
        address token1 = address(mockTokenA) < address(mockTokenB)
            ? address(mockTokenB)
            : address(mockTokenA);
        MockERC20 mockToken0 = MockERC20(token0);
        MockERC20 mockToken1 = MockERC20(token1);
        ...
        (uint128 longX, uint128 shortX, uint128 longY, uint128 shortY) = market
            .balanceOf(address(market), pairId);
        ...
        assertTrue(
            market.tokenBalances(token0) < initAmount * 2,
            "token0 balance should decrease"
        );
        assertTrue(
            market.tokenBalances(token1) < initAmount * 2,
    ...
    function test_WithdrawLiquidityRevertsInsufficientBurn() public {
        TestLiquidityCallback tokenCallback = new TestLiquidityCallback(market);
        InsufficientBurnCallback liquidityCallback = new InsufficientBurnCallback(
                market
            );
        MockERC20 mockTokenA = new MockERC20();
        MockERC20 mockTokenB = new MockERC20();
    ...
    function test_WithdrawLiquidityEdgeFullWithdraw() public {
        ...
        assertTrue(
            ...
            "Reserves should be > 0 after full withdraw"
        );
    ...
    function test_WithdrawLiquidityFuzz(
        uint256 w0L,
        uint256 w0S,
        uint256 w1L,
        uint256 w1S
        ...
        (uint128 r0L, uint128 r0S, uint128 r1L, uint128 r1S) = market
            .getReserves(token0, token1);
        assertTrue(
            r0L >= 1 && r0S >= 1 && r1L >= 1 && r1S >= 1,
            "Reserves >= min after withdraw"
        );
    }
    ...
    function test_SwapSingleHop() public {
        ...
        console.log(
            "SwapCallback token0 balance before:",
            mockToken0.balanceOf(address(swapCallback))
        );
        console.log(
            "Recipient token1 balance before:",
            mockToken1.balanceOf(to)
        );
        ...
        console.log(
            "SwapCallback token0 balance after:",
            mockToken0.balanceOf(address(swapCallback))
        );
        console.log(
            "Recipient token1 balance after:",
            mockToken1.balanceOf(to)
        );
        console.log(
            "Market token0 balance:",
            mockToken0.balanceOf(address(market))
        );
        console.log(
            "Market token1 balance:",
            mockToken1.balanceOf(address(market))
        );
        ...
        console.log(
            "Reserve0 change:",
            int256(uint256(r0L + r0S)) -
                int256(uint256(r0L_before + r0S_before))
        );
        console.log(
            "Reserve1 change:",
            int256(uint256(r1L + r1S)) -
                int256(uint256(r1L_before + r1S_before))
        );
        ...
        assertTrue(
            r0L + r0S > r0L_before + r0S_before,
            "Reserve0 should increase (we're giving token0)"
        );
        assertTrue(
            r1L + r1S < r1L_before + r1S_before,
            "Reserve1 should decrease (we're receiving token1)"
        );
    }
    ...
    function test_SwapMultiHop() public {
        ...
        assertEq(
            mockToken2.balanceOf(to),
            initialBalance2 + amountOut,
            "Recipient should receive tokens"
        );
    }
    ...
    function test_DebugLPBalance() public {
        ...
        // Check allowance
        uint256 allowanceValue = market.allowance(
            address(this),
            address(liquidityCallback),
            pairId
        );
        ...
        console.log(
            "Is allowance >= timestamp?",
            allowanceValue >= block.timestamp
        );

        // Check balance
        (uint128 longX3, , , ) = market.balanceOf(address(this), pairId);
        ...
        try liquidityCallback.requestLiquidity(address(0), pairId, 1, 1, 1, 1) {
            ...
            console.log(
                "Callback requestLiquidity failed with reason:",
                reason
            );
        } catch (bytes memory) {
            console.log(
                "Callback requestLiquidity failed with low-level error"
            );
        }
    }
    ...
    function test_SimpleTransferFrom() public {
        ...
        (uint128 longX1, , , ) = market.balanceOf(address(this), pairId);
        ...
        (uint128 longX2, , , ) = market.balanceOf(address(this), pairId);
    ...
    function test_DebugAllowanceTransferFrom() public {
        ...
        uint256 selfAllowanceValue = market.allowance(
            address(this),
            address(this),
            pairId
        );
        ...
        uint256 selfAllowanceAfter = market.allowance(
            address(this),
            address(this),
            pairId
        );
        ...
        // Try via callback
        try
            liquidityCallback.requestLiquidity(
                address(0xDEF),
                pairId,
                1,
                1,
    ...
    function test_SafeCast500000() public pure {
    ...
    function test_BurnFunction() public {
        ...
        (uint128 longXBefore, , , ) = market.balanceOf(address(this), pairId);
        (uint128 totalLongXBefore, , , ) = market.totalSupply(pairId);
        ...
        (uint128 longXAfter, , , ) = market.balanceOf(address(this), pairId);
        (uint128 totalLongXAfter, , , ) = market.totalSupply(pairId);

        // The user's balance should decrease by the burn amount
        assertEq(
            longXAfter,
            longXBefore - burnAmount,
    ...
    function test_DebugAllowanceIssue() public {
        ...
        uint256 allowanceBefore = market.allowance(
            address(this),
            address(liquidityCallback),
            pairId
        );
        ...
        uint256 allowanceAfter = market.allowance(
            address(this),
            address(liquidityCallback),
            pairId
        );
        ...
        console.log(
            "Is allowance >= block.timestamp?",
            allowanceAfter >= block.timestamp
        );
    ...
    function test_WithdrawLiquiditySmallAmount() public {
        ...
        assertEq(
            longX1,
            2000000 - withdraw,
            "Total supply should decrease by withdraw amount"
        );
    }
    ...
    function test_DebugCallbackTransferFrom() public {
        ...
        console.log(
            "Attempting direct transferFrom with amount:",
            transferAmount
        );
    ...
    function test_WithdrawLiquidityProtocolFee() public {
        ...
        (
            uint128 longXBefore,
            uint128 shortXBefore,
            uint128 longYBefore,
            uint128 shortYBefore
        ) = market.balanceOf(address(this), pairId);
        (
            uint128 totalLongXBefore,
            uint128 totalShortXBefore,
            uint128 totalLongYBefore,
            uint128 totalShortYBefore
        ) = market.totalSupply(pairId);
        ...
        (
            uint128 totalLongXAfter,
            uint128 totalShortXAfter,
            uint128 totalLongYAfter,
            uint128 totalShortYAfter
        ) = market.totalSupply(pairId);

        // The user's balance should decrease by the withdraw amount
        assertEq(
            longXAfter,
            longXBefore - withdrawAmount,
        ...
        uint256 expectedTotalSupply = totalLongXBefore -
            withdrawAmount +
            protocolFee;
        assertEq(
            totalLongXAfter,
            expectedTotalSupply,
            "Total supply should decrease by withdraw amount minus protocol fee"
        );
    }
}
